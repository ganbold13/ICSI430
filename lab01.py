...
def lcremovep(lst, pred):
    result = [e for e in lst if not pred(e)]
    return result
...
def lccollectp(lst, pred):
    result = [e for e in lst if pred(e)]
    return result
...
def power_set(lst):
    ...
    subsets_with_first = [[first_el] + subset for subset in subsets_without_first]

    result = subsets_with_first + subsets_without_first
...
def doubletree(tree):
    for i in range(len(tree)):
        if isinstance(tree[i], list):
            doubletree(tree[i])
        elif isinstance(tree[i], int) and not isinstance(tree[i], bool):
            tree[i] *= 2
...
def get_types(lst):
    ...
    for e in lst:
        if(isinstance(e, list)):
            types += get_types(e)
        elif not types.__contains__(type(e).__name__):
...
class queue:
    ...
    def __iter__(self):
        iter = queue(self.data)
        return (iter.data.pop(0) for d in range(len(iter.data)))
